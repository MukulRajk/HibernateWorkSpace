...
public class bankManagement { // these class provides all
				...
				int passCode) // create account function
	{
		try {
			...
			sql = "INSERT INTO customer_bank16 (cname,balance,pass_code) values('"
				+ name + "',1000," + passCode + ")";
	...
	loginAccount(String name, int passCode) // login method
	{
		try {
			...
			// query
			sql = "select * from customer_bank16 where cname='"
	...
	getBalance(int acNo) // fetch balance method
	{
		try {
			...
			// query
			sql = "select * from customer_bank16 where ac_no="
				+ acNo;
			PreparedStatement st
				= con.prepareStatement(sql);
		...
		throws SQLException // transfer money method
	{
		...
		try {
			...
			sql = "update customer_bank16 set balance=balance-"
